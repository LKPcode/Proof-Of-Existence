	...
	<div id="">
		<div class="nav-bar">
			...
			<div class="nav-links">
				...
				<button v-if="testAddress == undefined" class="nav-link connect-wallet-btn" v-on:click="connectWallet()" > Connect Wallet </button>
				<button v-else class="nav-link connect-wallet-btn" v-on:click="connectWallet()" > {{ shortenHash(this.testAddress)  }} </button>
		...
		<div class="content">
			<div class="level">
				...
				<div class="main-info left">
					...
					<p> Proof of ownership is the key to maintaining the authenticity to any of your most important documents. Secure any file on the blockchain following these simple steps.
			...
			<div class="level seccond-level" id="level2">
				...
				<div class="proof-info upload-info" >
					...
					<textarea v-model="doc_to_upload._hash" name="" id="hash-textarea" cols="30" rows="10" placeholder="SHA256"></textarea>
					
					<div class="upload-info-line"> Optional Message: </div>
					<textarea v-model="doc_to_upload.message" name="" id="" cols="30" rows="10" placeholder="Legal Contract about cholesterol levels of primates."></textarea>
					<div class="upload-info-line"> Estimated Datetime: <span style="font-weight:bold;font-size:medium;margin-left:20px;">{{this.doc_to_upload.est_datetime}}</span> </div>
				...
				<div class="main-info right">
					...
					<p> <span style="width:100px;"> 
						Create a permanent record of the existance of the file by uploading the hash to the blockchain.
...
Choose the file you would like to verify. Alternatively you can insert the hash of the file on the right where it says document hash.
Select verify document to see if the file/hash has been verified on the blockchain.
				...
				<div class="proof-info upload-info" >
					<div class="upload-info-line"> Document Hash: </div>
					<textarea v-model="doc_to_verify_hash" name="" id="hash-textarea" cols="30" rows="10" placeholder="Insert Hash to verify or Select a file"></textarea>
					
					<div class="upload-info-line"> Accompanied Message: </div>
					<textarea disabled v-model="this.verified_doc.message" name="" id="" cols="30" rows="10" placeholder="Message"></textarea>
					<div class="upload-info-line"> Document was created before: 
						<div v-if="this.verified_doc.error == false" style="color: greenyellow;margin-top:10px;;text-align: center;font-weight:bold;font-size:medium;margin-left:20px;">
							<span v-if="this.verified_doc.datetime != 0"> {{epochToHumanReadable(this.verified_doc.datetime)}}</span>
						</div> 
						<div v-else style="color: red ;margin-top:10px;text-align: center;font-weight:bold;font-size:medium;margin-left:20px;"> This File Does not Exist on The Blockchain </div> 
						
					</div>
					
					<button @click="verifyDocumentHash(doc_to_verify_hash)" class="make-permanent-btn">Verify Document</button>
